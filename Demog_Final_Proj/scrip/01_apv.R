...
# Gráfica pirámide 2010 con grupos quinquenales
apv2010[, age_group := cut(age, breaks = seq(0, 100, 5), 
                           labels = paste0(seq(0, 95, 5), "-", seq(4, 99, 5)),
                           right = FALSE)]
apv2010_quinquenal <- apv2010[, .(N = sum(N)), by = .(age_group, sex)]

ggplot(apv2010_quinquenal, aes(x = factor(age_group), 
                               y = ifelse(sex == "male", -N/1e6, N/1e6), 
                               fill = sex)) +
  ...
  labs(
    ...
    subtitle = "Distribución por edad y sexo (Grupos Quinquenales)",
    x = "Grupo de edad",
...
apv2019[, age_group := cut(age, breaks = seq(0, 100, 5), 
                           labels = paste0(seq(0, 95, 5), "-", seq(4, 99, 5)),
                           right = FALSE)]
apv2019_quinquenal <- apv2019[, .(N = sum(N)), by = .(age_group, sex)]

ggplot(apv2019_quinquenal, aes(x = factor(age_group), 
                               y = ifelse(sex == "male", -N/1e6, N/1e6), 
                               fill = sex)) +
  geom_col(width = 0.7, alpha = 0.8) +
  coord_flip() +
...
apv2021[, age_group := cut(age, breaks = seq(0, 100, 5), 
                           labels = paste0(seq(0, 95, 5), "-", seq(4, 99, 5)),
                           right = FALSE)]
apv2021_quinquenal <- apv2021[, .(N = sum(N)), by = .(age_group, sex)]

ggplot(apv2021_quinquenal, aes(x = factor(age_group), 
                               y = ifelse(sex == "male", -N/1e6, N/1e6), 
                               fill = sex)) +
  geom_col(width = 0.7, alpha = 0.8) +
  coord_flip() +
...
apv_comparison_quinquenal <- rbind(
  apv2010_quinquenal[, year := 2010],
  apv2019_quinquenal[, year := 2019],
  apv2021_quinquenal[, year := 2021]
)

ggplot(apv_comparison_quinquenal, aes(x = factor(age_group), 
                                      y = ifelse(sex == "male", -N/1e6, N/1e6), 
                                      fill = sex)) +
  ...
  labs(
    ...
    subtitle = "Grupos Quinquenales de Edad",
    x = "Grupo de edad",
